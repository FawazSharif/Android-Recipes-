...
import com.fawaz2.recipe.Adapters.SelectedMealAdapter
...
class RecipeSearchActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val `is`: InputStream = this.resources.openRawResource(R.raw.meals_names)

        var content = `is`.readBytes().toString(Charset.defaultCharset())
        var mealList = content.split("\n").toTypedArray()

        rec_Search.adapter = SelectedMealAdapter(mealList)
        ...
        searchText.addTextChangedListener(object : TextWatcher
        {
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                ...
                rec_Search.adapter = SelectedMealAdapter(filtered)
            }
